...
class DocumentHTMLSummarizer(OpenAIDocumentTransformer):
    '''
    Use OpenAI function calling to summarize the document to under a certain token limit.
    ...
    def __init__(self, *, config: DoctranConfig, token_limit: int) -> None:
        super().__init__(config)
        self.token_limit = token_limit
        self.function_name = "summarizehtml"
        self.function_description = f"Summarize an html document in under {self.token_limit} tokens. Html <a> tags should be included in the summary."
        self.function_parameters["properties"]["summary"] = {
            "type": "string",
            "description": "The summary of the html document. Html <a> tags should be included in the summary.",
        }
        self.function_parameters["required"].append("summary")
